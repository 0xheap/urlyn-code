...
// Japanese translations for Urlyn Code CLI
...
export default {
  ...
  'for help on Urlyn Code': 'Urlyn Code のヘルプ',
  'show version info': 'バージョン情報を表示',
  'submit a bug report': 'バグレポートを送信',
  'About Urlyn Code': 'Urlyn Code について',
  ...
  'List available Urlyn Code tools. Usage: /tools [desc]':
    '利用可能な Urlyn Code ツールを一覧表示。使い方: /tools [desc]',
  'List available skills.': '利用可能なスキルを一覧表示する。',
  'Available Urlyn Code CLI tools:': '利用可能な Urlyn Code CLI ツール:',
  ...
  'Generate with Urlyn Code (Recommended)': 'Urlyn Code で生成(推奨)',
  ...
  'To see changes, Urlyn Code must be restarted. Press r to exit and apply changes now.':
    '変更を確認するには Urlyn Code を再起動する必要があります。 r を押して終了し、変更を適用してください',
  'The command "/{{command}}" is not supported in non-interactive mode.':
    'コマンド "/{{command}}" は非対話モードではサポートされていません',
  'View and edit Urlyn Code settings': 'Urlyn Code の設定を表示・編集',
  Settings: '設定',
  'Vim Mode': 'Vim モード',
  ...
  'You are installing an extension from {{originSource}}. Some features may not work perfectly with Urlyn Code.':
    '{{originSource}} から拡張機能をインストールしています。一部の機能は Urlyn Code で完全に動作しない可能性があります。',
  ...
  'IDE integration is not supported in your current environment. To use this feature, run Urlyn Code in one of these supported IDEs: VS Code or VS Code forks.':
    '現在の環境ではIDE連携はサポートされていません。この機能を使用するには、VS Code または VS Code 派生エディタで Urlyn Code を実行してください',
  ...
  // Commands - Hooks
  // ============================================================================
  'Manage Urlyn Code hooks': 'Urlyn Code のフックを管理する',
  ...
  'This menu is read-only. To add or modify hooks, edit settings.json directly or ask Urlyn Code.':
    'このメニューは読み取り専用です。フックを追加または変更するには、settings.json を直接編集するか、Urlyn Code に尋ねてください。',
  ...
  'To re-enable hooks, remove "disableAllHooks" from settings.json or ask Urlyn Code.':
    'フックを再有効化するには、settings.json から "disableAllHooks" を削除するか、Urlyn Code に尋ねてください。',
  // Hooks - Source
  Project: 'プロジェクト',
  User: 'ユーザー',
  System: 'システム',
  ...
  "Urlyn Code won't ask before using allowed tools.":
    'Urlyn Code は許可されたツールを使用する前に確認しません。',
  'Urlyn Code will ask before using these tools.':
    'Urlyn Code はこれらのツールを使用する前に確認します。',
  'Urlyn Code is not allowed to use denied tools.':
    'Urlyn Code は拒否されたツールを使用できません。',
  ...
  'Urlyn Code can read files in the workspace, and make edits when auto-accept edits is on.':
    'Urlyn Code はワークスペース内のファイルを読み取り、自動編集承認が有効な場合は編集を行えます。',
  'Urlyn Code will be able to read files in this directory and make edits when auto-accept edits is on.':
    'Urlyn Code はこのディレクトリ内のファイルを読み取り、自動編集承認が有効な場合は編集を行えます。',
  ...
  'You can run any shell commands from Urlyn Code using ! (e.g. !ls).':
    'Urlyn Code から ! を使って任意のシェルコマンドを実行できます（例: !ls）。',
  ...
  'files to customize your interactions with Urlyn Code.':
    'Urlyn Code との対話をカスタマイズするためのファイル',
