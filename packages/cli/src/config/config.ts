...
export async function parseArguments(): Promise<CliArgs> {
  ...
  const yargsInstance = yargs(rawArgv)
    ...
    .scriptName('qwen')
    .usage(
      'Usage: qwen [options] [command]\n\nUrlyn Code - Launch an interactive CLI, use -p/--prompt for non-interactive mode',
    )
    .option('telemetry', {
      type: 'boolean',
      description:
    ...
    .option('proxy', {
      type: 'string',
      description:
        'Proxy for Urlyn Code, like schema://user:password@host:port',
    })
    .deprecateOption(
      'proxy',
    ...
    .command('$0 [query..]', 'Launch Urlyn Code CLI', (yargsInstance: Argv) =>
      yargsInstance
        .positional('query', {
          description:
