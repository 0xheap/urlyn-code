...
import { isGitRepository } from '@urlyn-code/urlyn-code-core';

vi.mock('@urlyn-code/urlyn-code-core', async (importOriginal) => {
  const actual =
    await importOriginal<typeof import('@urlyn-code/urlyn-code-core')>();
  return {
    ...actual,
...
describe('getInstallationInfo', () => {
  ...
  it('should detect global pnpm installation', () => {
    const pnpmPath = `/Users/test/.pnpm/global/5/node_modules/.pnpm/some-hash/node_modules/@urlyn-code/urlyn-code/dist/index.js`;
    ...
    expect(info.updateCommand).toBe(
      'pnpm add -g @urlyn-code/urlyn-code@latest',
    );
  ...
  it('should detect global yarn installation', () => {
    const yarnPath = `/Users/test/.yarn/global/node_modules/@urlyn-code/urlyn-code/dist/index.js`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe(
      'yarn global add @urlyn-code/urlyn-code@latest',
    );
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should detect global bun installation', () => {
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('bun add -g @urlyn-code/urlyn-code@latest');
    expect(info.updateMessage).toContain('Attempting to automatically update');
