...
import type { Config, SandboxConfig } from '@urlyn-code/urlyn-code-core';
import { FatalSandboxError } from '@urlyn-code/urlyn-code-core';
...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'urlyn-code-sandbox';
const SANDBOX_NETWORK_NAME = 'urlyn-code-sandbox';
const SANDBOX_PROXY_NAME = 'urlyn-code-sandbox-proxy';
...
export async function start_sandbox(
  ...
  // determine full path for urlyn-code to distinguish linked vs installed setting
  ...
  // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under urlyn-code repo
  //
  // note this can only be done with binary linked from urlyn-code repo
  if (process.env['BUILD_SANDBOX']) {
    if (!gcPath.includes('urlyn-code/packages/')) {
      throw new FatalSandboxError(
        'Cannot build sandbox using installed Urlyn Code binary; ' +
          'run `npm link ./packages/cli` under UrlynCode-cli repo to switch to linked binary.',
      );
    } else {
  ...
  if (!(await ensureSandboxImageIsPresent(config.command, image))) {
    const remedy =
      image === LOCAL_DEV_SANDBOX_IMAGE_NAME
        ? 'Try running `npm run build:all` or `npm run build:sandbox` under the urlyn-code repo to build it locally, or check the image name and your network connection.'
        : 'Please check the image name, your network connection, or notify urlyn-code-dev@service.alibaba.com if the issue persists.';
    throw new FatalSandboxError(
  ...
  const isIntegrationTest =
    process.env['URLYN_CODE_INTEGRATION_TEST'] === 'true';
  let containerName;
  if (isIntegrationTest) {
    containerName = `urlyn-code-integration-test-${randomBytes(4).toString(
      'hex',
    )}`;
  ...
  // copy URLYN_CODE_TEST_VAR for integration tests
  if (process.env['URLYN_CODE_TEST_VAR']) {
    args.push(
      '--env',
      `URLYN_CODE_TEST_VAR=${process.env['URLYN_CODE_TEST_VAR']}`,
    );
  }
