...
export const getLatestGitHubRelease = async (
  ...
  try {
    ...
    const endpoint = `https://api.github.com/repos/QwenLM/urlyn-code-action/releases/latest`;
    ...
    debugLogger.debug(
      `Failed to determine latest urlyn-code-action release:`,
      _error,
    );
    throw new Error(
      `Unable to determine the latest urlyn-code-action release on GitHub.`,
    );
  }
};
