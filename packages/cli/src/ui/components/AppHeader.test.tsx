...
const createMockConfig = (overrides = {}) => ({
  ...
  getTargetDir: vi.fn(() => '/projects/urlyn-code'),
  getMcpServers: vi.fn(() => ({})),
...
describe('<AppHeader />', () => {
  it('shows the working directory', () => {
    ...
    expect(lastFrame()).toContain('/projects/urlyn-code');
  });
  ...
  it('hides the header when screen reader is enabled', () => {
    ...
    expect(lastFrame()).not.toContain('/projects/urlyn-code');
    expect(lastFrame()).not.toContain('Urlyn Code');
  });

  it('shows the header with all info when banner is visible', () => {
    const { lastFrame } = renderWithProviders(createMockUIState());
    expect(lastFrame()).toContain('>_ Urlyn Code');
