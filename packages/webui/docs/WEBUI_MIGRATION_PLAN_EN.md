...
1. Extract components from `vscode-ide-companion/src/webview/` into an independent `@urlyn-code/webui` package
...
```typescript
// @urlyn-code/webui/src/context/PlatformContext.ts
...
```javascript
// @urlyn-code/webui/tailwind.preset.js
module.exports = {
  theme: {
    extend: {
      colors: {
...
module.exports = {
  presets: [require('@urlyn-code/webui/tailwind.preset')],
  content: [
    './src/**/*.{ts,tsx}',
    './node_modules/@urlyn-code/webui/dist/**/*.js',
  ],
};
```
...
```
Developer ──► @urlyn-code/webui ──► vscode-ide-companion
  │              │                      │
...
// After: Update import
import { FileIcon } from '@urlyn-code/webui';
// or import { FileIcon } from '@urlyn-code/webui/icons';
```
...
1. **Tailwind Class Name Tree Shaking**
   ...
   - Solution: Consumer's `content` config needs to include `node_modules/@urlyn-code/webui`
...
│     └── Edit files in @urlyn-code/webui/src/                     │
│                                                                  │
...
│     └── import { Component } from '@urlyn-code/webui'            │
...
1. **Single Source of Truth**: All UI components live in `@urlyn-code/webui`
