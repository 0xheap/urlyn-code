...
A minimum experimental TypeScript SDK for programmatic access to Urlyn Code.
...
```typescript
import { query } from '@urlyn-code/sdk';
...
Creates a new query session with the Urlyn Code.
...
| `env`                    | `Record<string, string>`                                                                                                                              | -                | Environment variables to pass to the Urlyn Code process. Merged with the current process environment.                                                                                                                                                                                                                                                                                                                                                                                   |
| `systemPrompt`           | `string \| QuerySystemPromptPreset`                                                                                                                   | -                | System prompt configuration for the main session. Use a string to fully override the built-in Urlyn Code system prompt, or a preset object to keep the built-in prompt and append extra instructions.                                                                                                                                                                                                                                                                                   |
...
| `authType`               | `'openai' \| 'anthropic' \| 'qwen-oauth' \| 'gemini' \| 'vertex-ai' \| 'ollama' \| 'groq' \| 'deepseek' \| 'mistral' \| 'together-ai' \| 'fireworks'` | `'openai'`       | Authentication type for the AI service. Using `'qwen-oauth'` in SDK is not recommended as credentials are stored in `~/.qwen` and may need periodic refresh.                                                                                                                                                                                                                                                                                                                            |
...
```typescript
import { query, type SDKUserMessage } from '@urlyn-code/sdk';
...
```typescript
import { query, type CanUseTool } from '@urlyn-code/sdk';
...
```typescript
import { z } from 'zod';
import { query, tool, createSdkMcpServer } from '@urlyn-code/sdk';
...
```typescript
import { query, isAbortError } from '@urlyn-code/sdk';
...
```typescript
import { AbortError, isAbortError } from '@urlyn-code/sdk';
...
#### Urlyn Code Installation Required

Version 0.1.0 requires [Urlyn Code](https://github.com/QwenLM/urlyn-code) **>= 0.4.0** to be installed separately and accessible in your PATH.

```bash
# Install Urlyn Code globally
npm install -g urlyn-code@^0.4.0
```

**Note**: From version **0.1.1** onwards, the CLI is bundled with the SDK, so no separate Urlyn Code installation is needed.
