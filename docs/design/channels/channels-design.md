...
> External messaging integrations for Urlyn Code — interact with an agent from Telegram, WeChat, and more.
>
> User documentation: [Channels Overview](../../users/features/channels/overview.md).

## Overview

A **channel** connects an external messaging platform to a Urlyn Code agent. Configured in `settings.json`, managed via `qwen channel` subcommands, multi-user (each user gets an isolated ACP session).
...
One `urlyn-code --acp` process with multiple ACP sessions. Scope per channel: **`user`** (default), **`thread`**, or **`single`**. Routing keys namespaced as `<channelName>:<key>`.
...
├── telegram/                # @urlyn-code/channel-telegram
├── weixin/                  # @urlyn-code/channel-weixin
└── dingtalk/                # @urlyn-code/channel-dingtalk
```
