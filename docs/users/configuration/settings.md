...
> Urlyn Code offers several ways to configure its behavior, including environment variables, command-line arguments, and settings files. This document outlines the different configuration methods and available settings.
...
Urlyn Code uses JSON settings files for persistent configuration. There are four locations for these files:

| File Type             | Location                                                                                                                                                                                                                                                                            | Scope                                                                                                                                                                                                                       |
| --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| System defaults file  | Linux: `/etc/urlyn-code/system-defaults.json`<br>Windows: `C:\ProgramData\urlyn-code\system-defaults.json`<br>macOS: `/Library/Application Support/UrlynCode/system-defaults.json` <br>The path can be overridden using the `URLYN_CODE_SYSTEM_DEFAULTS_PATH` environment variable. | Provides a base layer of system-wide default settings. These settings have the lowest precedence and are intended to be overridden by user, project, or system override settings.                                           |
| User settings file    | `~/.qwen/settings.json` (where `~` is your home directory).                                                                                                                                                                                                                         | Applies to all Urlyn Code sessions for the current user.                                                                                                                                                                    |
| Project settings file | `.qwen/settings.json` within your project's root directory.                                                                                                                                                                                                                         | Applies only when running Urlyn Code from that specific project. Project settings override user settings.                                                                                                                   |
| System settings file  | Linux： `/etc/urlyn-code/settings.json` <br>Windows: `C:\ProgramData\urlyn-code\settings.json` <br>macOS: `/Library/Application Support/UrlynCode/settings.json`<br>The path can be overridden using the `URLYN_CODE_SYSTEM_SETTINGS_PATH` environment variable.                    | Applies to all Urlyn Code sessions on the system, for all users. System settings override user and project settings. May be useful for system administrators at enterprises to have controls over users' Urlyn Code setups. |
...
In addition to a project settings file, a project's `.qwen` directory can contain other project-specific files related to Urlyn Code's operation, such as:
...
Urlyn Code automatically migrates legacy configuration settings to the new format. Old settings files are backed up before migration. The following settings have been renamed from negative (`disable*`) to positive (`enable*`) naming:
...
| `general.gitCoAuthor`           | boolean | Automatically add a Co-authored-by trailer to git commit messages when commits are made through Urlyn Code.                                                                     | `true`      |
...
| `enableWelcomeBack`                     | boolean          | Show welcome back dialog when returning to a project with conversation history. When enabled, Urlyn Code will automatically detect if you're returning to a project with a previously generated project summary (`.qwen/PROJECT_SUMMARY.md`) and show a dialog allowing you to continue your previous conversation or start fresh. This feature integrates with the `/summary` command and quit confirmation dialog. | `true`      |
...
Overrides the default context window size for the selected model. Urlyn Code determines the context window using built-in defaults based on model name matching, with a constant fallback value. Use this setting when a provider's effective context limit differs from Urlyn Code's default. This value defines the model's assumed maximum context capacity, not a per-request token limit.

**modalities:**

Overrides the auto-detected input modalities for the selected model. Urlyn Code automatically detects supported modalities (image, PDF, audio, video) based on model name pattern matching. Use this setting when the auto-detection is incorrect — for example, to enable `pdf` for a model that supports it but isn't recognized. Format: `{ "image": true, "pdf": true, "audio": true, "video": true }`. Omit a key or set it to `false` for unsupported types.
...
Configures connections to one or more Model-Context Protocol (MCP) servers for discovering and using custom tools. Urlyn Code attempts to connect to each configured MCP server to discover available tools. If multiple MCP servers expose a tool with the same name, the tool names will be prefixed with the server alias you defined in the configuration (e.g., `serverAlias__actualToolName`) to avoid conflicts. Note that the system might strip certain schema properties from MCP tool definitions for compatibility. At least one of `command`, `url`, or `httpUrl` must be provided. If multiple are specified, the order of precedence is `httpUrl`, then `url`, then `command`.
...
Configures logging and metrics collection for Urlyn Code. For more information, see [telemetry](/developers/development/telemetry).
...
Urlyn Code can automatically load environment variables from `.env` files.
...
| `DEBUG` or `DEBUG_MODE`        | (often used by underlying libraries or the CLI itself) Set to `true` or `1` to enable verbose debug logging, which can be helpful for troubleshooting. | **Note:** These variables are automatically excluded from project `.env` files by default to prevent interference with the CLI behavior. Use `.qwen/.env` files if you need to set these for Urlyn Code specifically.                                                                                                                                                                                                                                                              |
...
| `--prompt`                   | `-p`  | Used to pass a prompt directly to the command. This invokes Urlyn Code in a non-interactive mode.                                                                                       | Your prompt text                       | For scripting examples, use the `--output-format json` flag to get structured output.                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
By understanding and utilizing these configuration layers and the hierarchical nature of context files, you can effectively manage the AI's memory and tailor Urlyn Code's responses to your specific needs and projects.

## Sandbox

Urlyn Code can execute potentially unsafe operations (like shell commands and file modifications) within a sandboxed environment to protect your system.
...
By default, it uses a pre-built `urlyn-code-sandbox` Docker image.
...
When `.qwen/sandbox.Dockerfile` exists, you can use `BUILD_SANDBOX` environment variable when running Urlyn Code to automatically build the custom sandbox image:
...
To help us improve Urlyn Code, we collect anonymized usage statistics. This data helps us understand how the CLI is used, identify common issues, and prioritize new features.
