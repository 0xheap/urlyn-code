...
Urlyn Code supports three authentication methods. Pick the one that matches how you want to run the CLI:
...
- **How it works**: on first start, Urlyn Code opens a browser login page. After you finish, credentials are cached locally so you usually won't need to log in again.
...
- **How it works**: Subscribe to the Coding Plan with a fixed monthly fee, then configure Urlyn Code to use the dedicated endpoint and your subscription API key.
...
**Option B: Inside a Urlyn Code session**

Enter `qwen` in the terminal to launch Urlyn Code, then run the `/auth` command and select **Alibaba Cloud Coding Plan**. Choose your region, then enter your `sk-sp-xxxxxxxxx` key.
...
| `security.auth.selectedType` | Tells Urlyn Code which protocol to use on startup (e.g. `openai`, `anthropic`, `gemini`). Without this, you'd need to run `/auth` interactively. |
| `model.name`                 | The default model to activate when Urlyn Code starts. Must match one of the `id` values in your `modelProviders`.                                |
...
The key concept is **Model Providers** (`modelProviders`): Urlyn Code supports multiple API protocols, not just OpenAI. You configure which providers and models are available by editing `~/.qwen/settings.json`, then switch between them at runtime with the `/model` command.
...
Urlyn Code reads API keys from environment variables (specified by `envKey` in your model config). There are multiple ways to provide them, listed below from **highest to lowest priority**:
...
Urlyn Code auto-loads the **first** `.env` file it finds (variables are **not merged** across multiple files). Only variables not already present in `process.env` are loaded.

Search order (from the current directory, walking upward toward `/`):

1. `.qwen/.env` (preferred — keeps Urlyn Code variables isolated from other tools)
2. `.env`
...
> [!tip]
>
> `.qwen/.env` is recommended over `.env` to avoid conflicts with other tools. Some variables (like `DEBUG` and `DEBUG_MODE`) are excluded from project-level `.env` files to avoid interfering with Urlyn Code behavior.
...
After launching Urlyn Code, use the `/model` command to switch between all configured models. Models are grouped by protocol:
...
In addition to the in-session `/auth` slash command, Urlyn Code provides a standalone `qwen auth` CLI command for managing authentication directly from the terminal — without starting an interactive session first.
