# Urlyn Code Hooks Documentation

## Overview

Urlyn Code hooks provide a powerful mechanism for extending and customizing the behavior of the Urlyn Code application. Hooks allow users to execute custom scripts or programs at specific points in the application lifecycle, such as before tool execution, after tool execution, at session start/end, and during other key events.
...
Hooks are user-defined scripts or programs that are automatically executed by Urlyn Code at predefined points in the application flow. They allow users to:
...
The Urlyn Code hook system consists of several key components:
...
The following table lists all available hook events in Urlyn Code:
...
{
  "decision": "block",
  "reason": "Must be provided when Urlyn Code is blocked from stopping"
}
```
...
Hooks are configured in Urlyn Code settings, typically in `.qwen/settings.json` or user configuration files:
