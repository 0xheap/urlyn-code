# Connect Urlyn Code to tools via MCP

Urlyn Code can connect to external tools and data sources through the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/introduction). MCP servers give Urlyn Code access to your tools, databases, and APIs.

## What you can do with MCP

With MCP servers connected, you can ask Urlyn Code to:
...
Urlyn Code loads MCP servers from `mcpServers` in your `settings.json`. You can configure servers either:
...
3. Restart Urlyn Code in the same project (or start it if it wasn’t running yet), then ask the model to use tools from that server.
...
Use `includeTools` / `excludeTools` to restrict tools exposed by a server (from Urlyn Code’s perspective).
...
- **Environment variables in JSON don’t resolve**: ensure they exist in the environment where Urlyn Code runs (shell vs GUI app environments can differ).
