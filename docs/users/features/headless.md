...
Headless mode allows you to run Urlyn Code programmatically from command line
scripts and automation tools without any interactive UI. This is ideal for
scripting, automation, CI/CD pipelines, and building AI-powered tools.

## Overview

The headless mode provides a headless interface to Urlyn Code that:
...
Pipe input to Urlyn Code from your terminal:
...
Read from files and process with Urlyn Code:
...
Use `--system-prompt` to replace Urlyn Code's built-in main-session prompt for the current run:
...
Urlyn Code supports multiple output formats for different use cases:
...
The `--input-format` parameter controls how Urlyn Code consumes input from standard input:
