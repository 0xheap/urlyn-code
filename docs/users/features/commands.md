...
This document details all commands supported by Urlyn Code, helping you efficiently manage sessions, customize the interface, and control its behavior.

Urlyn Code commands are triggered through specific prefixes and fall into three categories:

| Prefix Type                | Function Description                                | Typical Use Case                                                 |
| -------------------------- | --------------------------------------------------- | ---------------------------------------------------------------- |
| Slash Commands (`/`)       | Meta-level control of Urlyn Code itself             | Managing sessions, modifying settings, getting help              |
...
Slash commands are used to manage Urlyn Code sessions, interface, and basic behavior.
...
| `/bug`      | Submit issue about Urlyn Code                   | `/bug Button click unresponsive` |
...
In addition to the in-session `/auth` slash command, Urlyn Code provides standalone CLI subcommands for managing authentication directly from the terminal:
...
> [!tip]
>
> These commands run outside of a Urlyn Code session. Use them to configure authentication before starting a session, or in scripts and CI environments. See the [Authentication](../configuration/auth) page for full details.
...
Exclamation commands allow you to execute system commands directly within Urlyn Code.
...
Environment Variables: Commands executed via `!` will set the `URLYN_CODE=1` environment variable.
