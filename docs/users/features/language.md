...
Urlyn Code is built for multilingual workflows: it supports UI localization (i18n/l10n) in the CLI, lets you choose the assistant output language, and allows custom UI language packs.

## Overview

From a user point of view, Urlyn Code’s “internationalization” spans multiple layers:
...
On first startup, Urlyn Code detects your system locale and sets the UI language automatically.

Detection priority:

1. `URLYN_CODE_LANG` environment variable
...
On first startup, if no `output-language.md` file exists, Urlyn Code automatically creates one based on your system locale. For example:
...
> [!note]
>
> After changing the output language, restart Urlyn Code for the change to take effect.
...
> For a concrete example, see [PR #1238: feat(i18n): add Russian language support](https://github.com/QwenLM/urlyn-code/pull/1238).
