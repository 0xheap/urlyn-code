...
This document explains how to run Urlyn Code inside a sandbox to reduce risk when tools execute shell commands or modify files.

## Prerequisites

Before using sandboxing, you need to install and set up Urlyn Code:
...
By default, Urlyn Code uses a published sandbox image (configured in the CLI package) and will pull it as needed.
...
If you don’t set either, Urlyn Code uses the default image configured in the CLI package (for example `ghcr.io/qwenlm/urlyn-code:<version>`).
...
On Linux, Urlyn Code defaults to enabling UID/GID mapping so the sandbox runs as your user (and reuses the mounted `~/.qwen`). Override with:
...
The official Urlyn Code Docker image is intentionally minimal to keep the image small, secure, and fast to pull. Different users require different language runtimes (Java, Python, Node.js, etc.), and bundling all environments into a single image is not practical. Therefore, Java is **not included by default** in the Docker sandbox.

If your workflow requires Java, you can extend the base image by creating a `.qwen/sandbox.Dockerfile` in your project:

```dockerfile
FROM ghcr.io/qwenlm/urlyn-code:latest
...
**Note:** If you have `DEBUG=true` in a project's `.env` file, it won't affect the CLI due to automatic exclusion. Use `.qwen/.env` files for Urlyn Code-specific debug settings.
