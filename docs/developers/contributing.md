...
```bash
git clone https://github.com/QwenLM/urlyn-code.git # Or your fork's URL
cd urlyn-code
```
...
To build both the `urlyn-code` CLI utility and the sandbox container, run `build:all` from the root directory:
...
To start the Urlyn Code application from the source code (after building), run the following command from the root directory:

```bash
npm start
```

If you'd like to run the source build outside of the urlyn-code folder, you can utilize `npm link path/to/urlyn-code/packages/cli` (see: [docs](https://docs.npmjs.com/cli/v9/commands/npm-link)) to run with `urlyn-code`
...
The integration tests are designed to validate the end-to-end functionality of Urlyn Code. They are not run as part of the default `npm run test` command.
...
- `packages/`: Contains the individual sub-packages of the project.
  ...
  - `core/`: The core backend logic for Urlyn Code.
...
**Note:** If you have `DEBUG=true` in a project's `.env` file, it won't affect urlyn-code due to automatic exclusion. Use `.urlyn-code/.env` files for urlyn-code specific debug settings.

### React DevTools

To debug the CLI's React-based UI, you can use React DevTools. Ink, the library used for the CLI's interface, is compatible with React DevTools version 4.x.

1.  **Start the Urlyn Code application in development mode:**
