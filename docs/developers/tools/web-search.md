...
Pass API keys when running Urlyn Code:
...
**Note:** This setting requires a restart of Urlyn Code to take effect. Once disabled, the `web_search` tool will not be available to the model, even if web search providers are configured.
...
- View your current settings in `~/.urlyn-code/settings.json` (macOS/Linux) or `%USERPROFILE%\.urlyn-code\settings.json` (Windows)
