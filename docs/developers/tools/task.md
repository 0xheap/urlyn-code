...
This document describes the `task` tool for Urlyn Code.
...
## How to use `task` with Urlyn Code
...
You can view available subagents by using the `/agents` command in Urlyn Code.
...
Subagents are configured through Urlyn Code's agent configuration system. Use the `/agents` command to:
