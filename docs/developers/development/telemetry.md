...
Learn how to enable and setup OpenTelemetry for Urlyn Code.
...
observability framework — Urlyn Code's observability system provides:
...
1. Enable telemetry in your `.qwen/settings.json`:
   ...
   ```
2. Run Urlyn Code and send prompts.
...
Logs are timestamped records of specific events. The following events are logged for Urlyn Code:

- `urlyn-code.config`: This event occurs once at startup with the CLI's configuration.
  - **Attributes**:
    - `model` (string)
...
- `urlyn-code.user_prompt`: This event occurs when a user submits a prompt.
  - **Attributes**:
    - `prompt_length` (int)
    - `prompt_id` (string)
...
- `urlyn-code.tool_call`: This event occurs for each function call.
  - **Attributes**:
    - `function_name`
    - `function_args`
    - `duration_ms`
...
- `urlyn-code.file_operation`: This event occurs for each file operation.
  - **Attributes**:
    - `tool_name` (string)
...
- `urlyn-code.api_request`: This event occurs when making a request to Qwen API.
  - **Attributes**:
    - `model`
    - `request_text` (if applicable)

- `urlyn-code.api_error`: This event occurs if the API request fails.
  - **Attributes**:
    - `model`
    - `error`
    - `error_type`
...
- `urlyn-code.api_response`: This event occurs upon receiving a response from Qwen API.
  - **Attributes**:
    - `model`
    - `status_code`
    - `duration_ms`
...
- `urlyn-code.tool_output_truncated`: This event occurs when the output of a tool call is too large and gets truncated.
  - **Attributes**:
    - `tool_name` (string)
...
- `urlyn-code.malformed_json_response`: This event occurs when a `generateJson` response from Qwen API cannot be parsed as a json.
  - **Attributes**:
    - `model`

- `urlyn-code.flash_fallback`: This event occurs when Urlyn Code switches to flash as fallback.
  - **Attributes**:
    - `auth_type`

- `urlyn-code.slash_command`: This event occurs when a user executes a slash command.
  - **Attributes**:
    - `command` (string)
    - `subcommand` (string, if applicable)

- `urlyn-code.extension_enable`: This event occurs when an extension is enabled
- `urlyn-code.extension_install`: This event occurs when an extension is installed
  - **Attributes**:
    - `extension_name` (string)
    - `extension_version` (string)
    - `extension_source` (string)
    - `status` (string)
- `urlyn-code.extension_uninstall`: This event occurs when an extension is uninstalled

### Metrics

Metrics are numerical measurements of behavior over time. The following metrics are collected for Urlyn Code (metric names remain `urlyn-code.*` for compatibility):

- `urlyn-code.session.count` (Counter, Int): Incremented once per CLI startup.

- `urlyn-code.tool.call.count` (Counter, Int): Counts tool calls.
  - **Attributes**:
    - `function_name`
    - `success` (boolean)
...
- `urlyn-code.tool.call.latency` (Histogram, ms): Measures tool call latency.
  - **Attributes**:
    - `function_name`
    - `decision` (string: "accept", "reject", or "modify", if applicable)

- `urlyn-code.api.request.count` (Counter, Int): Counts all API requests.
  - **Attributes**:
    - `model`
    - `status_code`
    - `error_type` (if applicable)

- `urlyn-code.api.request.latency` (Histogram, ms): Measures API request latency.
  - **Attributes**:
    - `model`

- `urlyn-code.token.usage` (Counter, Int): Counts the number of tokens used.
  - **Attributes**:
    - `model`
    - `type` (string: "input", "output", "thought", "cache", or "tool")

- `urlyn-code.file.operation.count` (Counter, Int): Counts file operations.
  - **Attributes**:
...
- `urlyn-code.chat_compression` (Counter, Int): Counts chat compression operations
  - **Attributes**:
